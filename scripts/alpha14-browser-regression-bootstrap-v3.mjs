...
const pageSyncReplacement=`await ev(\`(()=>{const s=document.getElementById('readerPreviewDevice');s.value='phone-390';s.dispatchEvent(new Event('change',{bubbles:true}));document.querySelector('.page-item[data-page-index="4"]').click()})()\`);let pageSynced=false;for(let i=0;i<80;i++){pageSynced=await ev("window.__V3_STUDIO__.state.page===4 && document.getElementById('builtPreviewFrame').contentWindow.__V3_STATE__.pageIndex===4").catch(()=>false);if(pageSynced)break;await sleep(25)}assert(pageSynced,'制作中心页码没有通过 request/ack 同步到 Reader');`;

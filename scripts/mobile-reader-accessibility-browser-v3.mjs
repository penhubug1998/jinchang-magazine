...
try{
  ...
  const initial=await evaluate(`(()=>{const s=window.__V3_STATE__,button=document.getElementById('seniorButton'),root=getComputedStyle(document.documentElement),actions=[...document.querySelectorAll('.actions button')].map(x=>x.getBoundingClientRect());return {mode:s.networkMode,banner:!document.getElementById('networkStatus').hidden,senior:s.seniorMode,pressed:button.getAttribute('aria-pressed'),font:parseFloat(root.getPropertyValue('--page-font-base')),overflow:document.documentElement.scrollWidth>innerWidth+1,actions:actions.map(r=>({w:r.width,h:r.height,l:r.left,r:r.right}))};})()`);
  assert(initial.mode==='constrained'&&initial.banner,`2G mode failed ${JSON.stringify(initial)}`);assert(!initial.senior&&initial.pressed==='false',`senior initial state failed ${JSON.stringify(initial)}`);assert(!initial.overflow,`390px horizontal overflow ${JSON.stringify(initial)}`);assert(initial.actions.length===5&&initial.actions.every(r=>r.h>=43.5&&r.l>=-1&&r.r<=391),`390px action targets failed ${JSON.stringify(initial.actions)}`);
  await evaluate(`document.getElementById('seniorButton').click()`);await sleep(180);
  const senior=await evaluate(`(()=>{const s=window.__V3_STATE__,button=document.getElementById('seniorButton'),root=getComputedStyle(document.documentElement),p=document.querySelector('.page p,.rich-text-block p');return {active:s.seniorMode,body:document.body.classList.contains('senior-mode'),pressed:button.getAttribute('aria-pressed'),font:parseFloat(root.getPropertyValue('--page-font-base')),line:p?parseFloat(getComputedStyle(p).lineHeight):0,toolbar:document.querySelector('.toolbar').getBoundingClientRect().height};})()`);
  assert(senior.active&&senior.body&&senior.pressed==='true',`senior toggle failed ${JSON.stringify(senior)}`);assert(senior.font>initial.font,`senior font did not increase ${JSON.stringify({initial:initial.font,senior:senior.font})}`);assert(senior.toolbar>=63,`senior toolbar too small ${JSON.stringify(senior)}`);
  ...
  const small=await evaluate(`(()=>{const toolbar=document.querySelector('.toolbar').getBoundingClientRect(),actions=[...document.querySelectorAll('.actions button')].map(x=>x.getBoundingClientRect());return {w:innerWidth,overflow:document.documentElement.scrollWidth>innerWidth+1,toolbar:{l:toolbar.left,r:toolbar.right,b:toolbar.bottom,h:toolbar.height},actions:actions.map(r=>({l:r.left,r:r.right,h:r.height}))};})()`);
  assert(!small.overflow&&small.toolbar.l>=-1&&small.toolbar.r<=small.w+1&&small.toolbar.b<=innerHeight+1,`320px toolbar bounds failed ${JSON.stringify(small)}`);assert(small.actions.every(r=>r.l>=-1&&r.r<=small.w+1&&r.h>=43.5),`320px actions bounds failed ${JSON.stringify(small.actions)}`);
  console.log('P1-06 Reader browser PASS: 390/320 mobile bounds, senior mode readability, 2G/offline/recovery state, and retryable image failures all work.');
}finally{

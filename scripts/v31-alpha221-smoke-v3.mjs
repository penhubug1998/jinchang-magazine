...
import { legacyTextToRichText, normalizeRichText, renderRichText, richTextPlainText, sanitizePastedHtml, sanitizePastedText, RICH_TEXT_ENGINE_INFO } from '../src/reader/rich-text.js';
...
const legacy=legacyTextToRichText('银龄初心');assert(richTextPlainText(legacy)==='银龄初心','legacy mirror failed');assert(renderRichText(normalizeRichText(null,'兼容')).includes('兼容'),'legacy render failed');
const schema=JSON.parse(await readFile('baselines/v3-schema-3.1-alpha22.1.json','utf8'));assert(schema.richText?.selfHostedRuntime&&schema.richText?.externalCdnFallback===false&&schema.history?.studioSessionMerge&&schema.paste?.sanitizeHtml&&schema.selectionToolbar?.floatingBubble,'alpha22.1 schema contract missing');
const lock=JSON.parse(await readFile('baselines/v3.0-final-gate-lock-alpha22.1.json','utf8'));for(const [file,digest] of Object.entries(lock.files))assert(sha(await readFile(file))===digest,`V3.0 gate lock changed: ${file}`);
const [rt,reader,readerHtml,readerCss,studio,server,vendorScript,vendorSource]=await Promise.all(['src/reader/rich-text.js','src/reader/reader.js','src/reader/index.html','src/reader/reader.css','src/studio/studio.js','scripts/studio-v3.mjs','scripts/vendor-tiptap-v3.mjs','src/reader/vendor/tiptap-runtime.js'].map(x=>readFile(x,'utf8')));
assert(!rt.includes('https://esm.sh')&&!rt.includes('https://cdn.jsdelivr')&&!rt.includes('https://unpkg'),'Reader runtime still references external CDN');
for(const token of ['LOCAL_VENDOR_MODULE','sanitizePastedHtml','createNativeStructuredEditor','externalCdnFallback:false'])assert(rt.includes(token),`rich-text missing ${token}`);
for(const token of ['data-rich-command="undo"','data-rich-command="redo"','studioRichBubble','data-rich-bubble-command="bold"'])assert(readerHtml.includes(token),`reader toolbar missing ${token}`);
for(const token of ['sessionId:`rt-','positionRichBubble','runRichCommand','getRichTextRuntimeStatus'])assert(reader.includes(token),`reader missing ${token}`);
assert(readerCss.includes('V3.1-alpha22.1 · Rich Text production hardening'),'reader hardening CSS marker missing');
assert(studio.includes('recordHistory=true')&&studio.includes('recordHistory:Boolean(data.final)')&&studio.includes('canvas-richtext:${String(data.sessionId||block.id)}'),'Studio rich-text history merge missing');
assert(server.includes("rest.startsWith('vendor/')"),'live preview vendor route missing');
...
console.log('V3.1-alpha22.1 smoke 通过：真实自托管 Tiptap runtime、结构化离线 fallback、粘贴清洗、选区工具条与单会话 Undo/Redo 合并链路正常。');

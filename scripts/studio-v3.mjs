...
async function publicationStatus(id,{refreshAudit=true}={}){
  const issue=await readJson(path.join(root,'issues',id,'issue.json'));const {audit}=await readPublicationAudit(id,{refresh:refreshAudit});const evidence=await readPublicationEvidence(id);const status=buildPublicationStatus(issue,audit,evidence);
  return {...status,publicShare:{configured:Boolean(publicMagazineRoot&&publicMagazineBaseUrl),url:publicMagazineBaseUrl?`${publicMagazineBaseUrl}/${publicIssuePath(id)}/`:null,archiveUrl:publicMagazineBaseUrl?`${publicMagazineBaseUrl}/`:null},publicDeployment:evidence.publicDeployment||null};
}
...
async function publicationPrintFixture(id){
  const issue=await readJson(path.join(root,'issues',id,'issue.json'));const plan=buildPublishingPlan(issue);const articles=issue.articles&&typeof issue.articles==='object'?issue.articles:{};
...
async function generatePublicationPdf(id){
  await ensurePublicationWeb(id);const chromium=findChromium();if(!chromium)throw new Error('未找到 Chromium，无法生成 Print PDF');const dir=path.join(PUBLICATION_OUTPUT_ROOT,id);await mkdir(dir,{recursive:true});const file=path.join(dir,`${id}-print.pdf`);await rm(file,{force:true});const fixture=await publicationPrintFixture(id);const userDataDir=await mkdtemp(path.join(os.tmpdir(),'jinchang-pdf-'));const debugPort=13200+Math.floor(Math.random()*500);let chrome=null,ws=null;const pending=new Map();let seq=0;const cdpSend=(method,params={},timeoutMs=15000)=>new Promise((resolve,reject)=>{const requestId=++seq;pending.set(requestId,{resolve,reject});ws.send(JSON.stringify({id:requestId,method,params}));setTimeout(()=>{const p=pending.get(requestId);if(p){pending.delete(requestId);reject(new Error(`CDP ${method} 超时`));}},timeoutMs);});
  try{chrome=spawn('xvfb-run',['-a',chromium,'--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--remote-allow-origins=*',`--remote-debugging-port=${debugPort}`,`--user-data-dir=${userDataDir}`,'--no-first-run','about:blank'],{stdio:'ignore',detached:true});let tabs=null;for(let i=0;i<100;i++){try{const r=await fetch(`http://127.0.0.1:${debugPort}/json/list`);if(r.ok){tabs=await r.json();if(tabs?.length)break;}}catch{}await new Promise(r=>setTimeout(r,80));}if(!tabs?.length)throw new Error('Chromium PDF 进程未能启动调试端口');const tab=tabs.find(x=>x.type==='page')||tabs[0];ws=new WebSocket(tab.webSocketDebuggerUrl);await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error('连接 Chromium CDP 超时')),8000);ws.onopen=()=>{clearTimeout(timer);resolve();};ws.onerror=()=>{clearTimeout(timer);reject(new Error('连接 Chromium CDP 失败'));};});ws.onmessage=e=>{const m=JSON.parse(e.data);if(!m.id)return;const p=pending.get(m.id);if(!p)return;pending.delete(m.id);m.error?p.reject(new Error(m.error.message)):p.resolve(m.result);};await cdpSend('Page.enable');const tree=await cdpSend('Page.getFrameTree');await cdpSend('Page.setDocumentContent',{frameId:tree.frameTree.frame.id,html:fixture});const pdf=await cdpSend('Page.printToPDF',{printBackground:true,preferCSSPageSize:true,paperWidth:8.2677165,paperHeight:11.6929134,marginTop:0,marginBottom:0,marginLeft:0,marginRight:0,displayHeaderFooter:false,transferMode:'ReturnAsStream'},90000);if(!pdf?.stream)throw new Error('Chromium 未返回 PDF 数据流');const chunks=[];let eof=false,total=0;while(!eof){const part=await cdpSend('IO.read',{handle:pdf.stream,size:1024*1024},30000);if(part?.data){const chunk=Buffer.from(part.data,part.base64Encoded?'base64':'utf8');chunks.push(chunk);total+=chunk.length;if(total>128*1024*1024)throw new Error('Print PDF 超过 128MB 安全上限');}eof=Boolean(part?.eof);}await cdpSend('IO.close',{handle:pdf.stream},5000).catch(()=>{});if(!chunks.length)throw new Error('Chromium 返回了空 PDF 数据流');await writeFile(file,Buffer.concat(chunks));}finally{try{ws?.close()}catch{}if(chrome?.pid)try{process.kill(-chrome.pid,'SIGTERM')}catch{}await new Promise(r=>setTimeout(r,120));await rm(userDataDir,{recursive:true,force:true}).catch(()=>{});}if(!(await exists(file)))throw new Error('Print PDF 文件未生成');const info=await stat(file);const manifest={kind:'pdf',generatedAt:new Date().toISOString(),path:path.relative(root,file).replaceAll('\\','/'),url:`/publication-output/${id}/${path.basename(file)}`,bytes:info.size,sha256:await sha256(file)};await writePublicationEvidence(id,{outputs:{pdf:manifest}});return manifest;
}
...
const server=http.createServer(async(req,res)=>{try{
  ...
  if (u.pathname==='/api/import/parse'&&req.method==='POST') {
    ...
    } catch(e){return send(res,400,{error:e.message||String(e),code:'IMPORT_ERROR'});}
  }

...
async function generatePublicationPdf(id){
  ...
  await mkdir(dir,{recursive:true});await rm(file,{force:true});
  ...
  try{
    ...
    await cdpSend('IO.close',{handle:pdf.stream},5000).catch(()=>{});if(!chunks.length)throw publicationFailure('PDF_EMPTY','Chromium 返回了空 PDF 数据流',['请检查 Chromium 运行环境后重试。']);await writeFile(file,Buffer.concat(chunks));
  }finally{try{ws?.close()}catch{}if(chrome?.pid)try{process.kill(-chrome.pid,'SIGTERM')}catch{}await new Promise(resolve=>setTimeout(resolve,120));await rm(userDataDir,{recursive:true,force:true}).catch(()=>{})}

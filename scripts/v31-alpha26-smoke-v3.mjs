...
try{let health;for(let i=0;i<150;i++){try{const r=await fetch(`http://127.0.0.1:${port}/api/health`);if(r.ok){health=await r.json();break}}catch{}await sleep(40)}assert(['3.1.0-alpha.26','3.1.0-beta.1','3.1.0-rc.1','3.1.0-rc.2','3.1.0'].includes(health?.version),`health ${JSON.stringify(health)} ${logs}`);const workspace=await fetch(`http://127.0.0.1:${port}/workspace/?issue=001&page=1&mobile=1`);assert(workspace.ok&&(await workspace.text()).includes('mobileStudioDock'),'workspace mobile shell route failed');const module=await fetch(`http://127.0.0.1:${port}/mobile-studio.js`);assert(module.ok&&(await module.text()).includes('MOBILE_STUDIO_BREAKPOINT'),'mobile helper route failed');}

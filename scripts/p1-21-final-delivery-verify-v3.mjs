...
import { verifyFinalDeliveryObject } from './lib-p1-21-final-delivery-v3.mjs';
...
const artifact=await verifyFinalReleaseObject(release,{sourceCommit:source.commit,bundleSha256:manifest.bundleSha256,manifestSha256:manifest.manifestSha256});
errors.push(...artifact.errors);
const delivery=await verifyFinalDeliveryObject(release,{sourceCommit:source.commit,webArtifact:artifact.artifact});
errors.push(...delivery.errors);
if(errors.length){console.error(`P1-21 final delivery verify FAILED\n${errors.join('\n')}`);process.exit(1)}
console.log(`P1-21 final delivery verify PASS\nsource=${source.commit}\nissue=${delivery.delivery.issue}\nweb=${delivery.delivery.web.treeSha256}\npdf=${delivery.delivery.pdf.sha256}\narchive=${delivery.delivery.archive.sha256}\ndelivery=${delivery.delivery.deliverySha256}\nrelease=${release.releaseSha256}`);

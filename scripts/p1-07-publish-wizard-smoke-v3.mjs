...
const port=5400+Math.floor(Math.random()*300),child=spawn(process.execPath,['scripts/studio-v3.mjs','--host','127.0.0.1','--port',String(port),'--acceptance-only'],{stdio:'ignore'});
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
async function waitJson(url,timeout=12000){const t=Date.now();let last;while(Date.now()-t<timeout){try{const r=await fetch(url);if(r.ok)return await r.json()}catch(e){last=e}await sleep(80)}throw last||new Error(`timeout ${url}`)}
try{
  await waitJson(`http://127.0.0.1:${port}/api/health`);
  const wf=await waitJson(`http://127.0.0.1:${port}/api/issues/001/publication/workflow?refresh=0`);

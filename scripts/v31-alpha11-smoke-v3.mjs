...
assert(['3.1.0-alpha.11','3.1.0-alpha.12','3.1.0-alpha.13','3.1.0-alpha.14','3.1.0-alpha.15','3.1.0-alpha.16','3.1.0-alpha.17','3.1.0-alpha.18','3.1.0-alpha.19','3.1.0'].includes(pkg.version),`Alpha11 版本错误：${pkg.version}`);
assert(['3.1-alpha11','3.1-alpha12','3.1-alpha13','3.1-alpha14','3.1-alpha15','3.1-alpha16','3.1-alpha17','3.1-alpha18','3.1-alpha19','3.1-alpha24'].includes(pkg.v31SchemaVersion),`Alpha11 schema 版本错误：${pkg.v31SchemaVersion}`);
...
for(const id of ['immersiveWorkspaceToolbar','workspaceBackBtn','workspacePagePicker','workspacePageDialog','managerWorkspaceEnter'])assert(html.includes(`id="${id}"`),`Alpha11 UI 缺少 ${id}`);
...
for(const token of ['WORKSPACE_MODE','workspaceRouteUrl','goToPage','openWorkspacePageDialog','function scheduleReaderPreviewSync('])assert(studio.includes(token),`Alpha11 Studio 缺少 ${token}`);
assert(studio.indexOf("frame.contentWindow?.postMessage({source:'v3-studio',type:'issue'")<studio.indexOf('await api(`/api/issues/${encodeURIComponent(issueId)}/live-preview`'),'Alpha11 应先即时 postMessage 再等待 live-preview 后台持久化');
...
try{
  const health=await (await wait(`http://127.0.0.1:${port}/api/health`)).json();assert(health.ok&&(health?.version===pkg.version),`Alpha11 health 异常：${JSON.stringify(health)}`);
  ...
  let r=await fetch(`http://127.0.0.1:${port}/workspace/?issue=001&page=3`);const ws=await r.text();assert(r.ok&&ws.includes('immersiveWorkspaceToolbar')&&ws.includes('id="studioVersionLabel"'),'Workspace HTML 路由异常');

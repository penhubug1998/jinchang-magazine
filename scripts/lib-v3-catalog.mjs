...
export function buildArchiveHtml(catalog, options={}) {
  ...
  // 按用户分区：同一个模板同时渲染平台归档页（/）、作者空间索引（/u/）
  // 和单个作者的归档页（/u/<slug>/）。spaces 只在前两处出现。
  const spaces = Array.isArray(options.spaces) ? options.spaces : [];
  const navItems = Array.isArray(options.nav) ? options.nav : [];
  ...
  const spaceCards = spaces.map((space) => `
    <a class="space-card" href="${esc(space.href)}" data-issues="${Number(space.issueCount) || 0}">
      <b>${esc(space.journalName || space.displayName || space.username || space.slug)}</b>
      <span>${esc(space.displayName || space.username || space.slug)} · ${Number(space.issueCount) || 0} 期已发布</span>
      <span class="open">查看该作者期刊 →</span>
    </a>`).join('\n');
  const nav = navItems.length ? `<nav class="topnav">${navItems.map(item => `<a href="${esc(item.href)}">${esc(item.label)}</a>`).join('')}</nav>` : '';
  const spacesSection = spaces.length ? `<section class="spaces"><h2 class="section-title">作者期刊空间</h2><div class="space-grid">${spaceCards}</div></section>` : '';
  return `<!doctype html>

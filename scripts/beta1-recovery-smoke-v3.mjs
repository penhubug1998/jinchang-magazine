...
try{
  ...
  // 失败发布必须保留上一份可用 release。不要依赖易变的控制台文案，直接验证严格审计的机器可读 blocker code。
  issue=JSON.parse(await readFile(issueFile,'utf8'));issue.status='draft';await writeFile(issueFile,JSON.stringify(issue,null,2)+'\n');run('publish-v3.mjs',['--issue','001','--skip-browser'],1);
  const failedAudit=JSON.parse(await readFile(path.join(sandbox,'reports','v3-release-audit-001.json'),'utf8'));
  const failedIssueAudit=failedAudit.issues?.find(x=>x.id==='001');
  assert(failedIssueAudit?.blockers?.some(x=>x.code==='STATUS_NOT_READY'),`严格发布失败未产生 STATUS_NOT_READY：${JSON.stringify(failedIssueAudit?.blockers||[])}`);
  const afterFailed=await digestDir(releaseDir);assert(afterFailed===goodRelease,'失败发布破坏了上一份 release 目录');

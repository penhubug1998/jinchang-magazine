...
try{
  ...
  // 失败发布必须保留上一份可用 release。严格发布门禁现在由 audit 的 STATUS_NOT_READY 语义负责。
  issue=JSON.parse(await readFile(issueFile,'utf8'));issue.status='draft';await writeFile(issueFile,JSON.stringify(issue,null,2)+'\n');const failed=run('publish-v3.mjs',['--issue','001','--skip-browser'],1);const failureOutput=`${failed.stderr}\n${failed.stdout}`;assert(/STATUS_NOT_READY|严格发布检查要求 status=ready 或 published|状态未进入发布阶段/.test(failureOutput),`失败原因不是预期的发布状态门禁：\n${failureOutput}`);const afterFailed=await digestDir(releaseDir);assert(afterFailed===goodRelease,'失败发布破坏了上一份 release 目录');

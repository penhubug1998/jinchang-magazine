import { normalizeRichText, renderRichText, richTextPlainText, createRichTextEditor, getRichTextRuntimeStatus, RICH_TEXT_ENGINE_INFO } from './rich-text.js?v=3.1.0-rc.1';
import { buildPublishingPlan, flowFragmentFor, normalizePagePublishing, publishingBlockClass, publishingBlockStyle, pagePublishingStyle, LAYOUT_ENGINE_INFO } from './layout-engine.js?v=3.1.0-rc.1';
...
async function requestElementFullscreen(el) {
  ...
  if (!fn) return false;
  try { await fn.call(el); return Boolean(fullscreenElement()); } catch { return false; }
}
...
function applyIssueDesign(issue){
  ...
  root.setProperty('--paper','#fffaf0');root.setProperty('--ink','#3b2d26');root.setProperty('--red','#8d1f1c');root.setProperty('--design-muted','#8a7566');root.setProperty('--page-font-base','13.3px');root.setProperty('--design-radius','12px');root.setProperty('--design-spacing','10px');
...
function renderPage(page, index = -1) {
  ...
  const pageNo = index > 0 ? `<span class="pageno">${String(index).padStart(2, "0")}</span>` : "";
  const pStyle=pageDesignStyle(page), scrollStyle=pageScrollDesignStyle(page);
  return `<article class="page ${escapeHtml(type)}" data-page-type="${escapeHtml(type)}" data-page-index="${index}" data-page-id="${escapeHtml(page.id || '')}"${pStyle?` style="${escapeHtml(pStyle)}"`:""}><div class="texture" aria-hidden="true"></div><div class="page-scroll"${scrollStyle?` style="${escapeHtml(scrollStyle)}"`:""}>
...
function spreadHtml(index) {
  return `<div class="spread">${spreadAt(index).map((entry) => renderPage(entry.page, entry.index)).join("")}</div>`;
}
...
function animateMove(direction) {
  ...
  stopNarration();
  if (!state.issue.features?.flipAnimation || matchMedia("(prefers-reduced-motion: reduce)").matches) {
    state.pageIndex = target;
    render();
    return;
  }
  ...
  state.turning = true;
  const stage = $("stage");
  const oldSpread = spreadAt(state.pageIndex);
  const newSpread = spreadAt(target);
  let frontPage, backPage, baseLeft, baseRight, turnClass;
  if (direction > 0) {
    baseLeft = oldSpread[0];
    baseRight = newSpread[1];
    frontPage = oldSpread[1];
    backPage = newSpread[0];
    turnClass = "next";
  } else {
    baseLeft = newSpread[0];
    baseRight = oldSpread[1];
    frontPage = oldSpread[0];
    backPage = newSpread[1];
    turnClass = "prev";
  }
  stage.innerHTML = `<div class="spread">${renderPage(baseLeft.page, baseLeft.index)}${renderPage(baseRight.page, baseRight.index)}</div><div class="turner ${turnClass}"><div class="turn-face front">${renderPage(frontPage.page, frontPage.index)}</div><div class="turn-face back">${renderPage(backPage.page, backPage.index)}</div></div>`;
  requestAnimationFrame(() => requestAnimationFrame(() => stage.querySelector(".turner")?.classList.add("go")));
  setTimeout(() => {
    state.pageIndex = target;
    state.turning = false;
    render();
...
function pageSpeechText(index) {
  ...
  for (const block of page?.blocks || []) {
    if (["paragraph", "blessing", "producer", "coverMeta"].includes(block.type)) pieces.push(block.text);
    if (block.type === "coverSections") pieces.push(...(block.items || []));
    if (block.type === "quote") pieces.push(block.title, block.text);
    if (block.type === "cardline") pieces.push(block.title, block.text);
    if (block.type === "casePair") pieces.push("案例", block.case, "警示", block.warning);
    if (block.type === "chips") pieces.push(...(block.items || []).map((item) => item.text));
  }
...
function openVideoFullscreen(button) {
  const frame = button.closest(".video-frame");
  const video = frame?.querySelector("video");
  if (!frame || !video) return;
  // iPhone Safari exposes native video fullscreen instead of document fullscreen.
  ...
  $("videoFullscreenOverlay").classList.add("open");
  $("videoFullscreenOverlay").setAttribute("aria-hidden", "false");
}

function closeVideoFullscreen() {
  const overlay = $("videoFullscreenOverlay");
  const video = $("videoFullscreenSlot").querySelector("video");
  if (video && state.videoOrigin?.isConnected) state.videoOrigin.insertBefore(video, state.videoOrigin.firstChild);
  else video?.remove();
...
function prepareMobileSwipe(direction, target) {
  ...
  stage.innerHTML = `<div class="spread">${renderPage(targetPage, target)}</div><div class="swipe-layer ${direction > 0 ? "forward" : "backward"}">${renderPage(currentPage, state.pageIndex)}</div>`;
}
...
function handlePointerMove(event) {
  ...
  pointer.raf = requestAnimationFrame(() => {
    ...
    if (!overlay) return;
    const angle = pointer.direction > 0 ? -165 * progress : 165 * progress;
    overlay.style.transform = `translate3d(0,0,0) rotateY(${angle}deg)`;
    overlay.style.opacity = String(1 - progress * .02);
  });
}
...
function finishSwipe(commit, progress = 0) {
  ...
  const duration = commit ? Math.max(170, Math.round((1 - progress) * 360)) : Math.max(140, Math.round(progress * 260));
  overlay.style.transition = `transform ${duration}ms cubic-bezier(.22,.61,.36,1), opacity ${duration}ms ease`;
  overlay.style.transform = commit ? `translate3d(0,0,0) rotateY(${pointer.direction > 0 ? -165 : 165}deg)` : "translate3d(0,0,0) rotateY(0deg)";
  overlay.style.opacity = commit ? ".98" : "1";
  state.turning = true;
  setTimeout(() => {
    if (commit) {
      stopNarration();
...
function bind() {
  ...
  $("readButton").addEventListener("click", () => $("readDialog").showModal());

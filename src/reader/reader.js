...
function designTargetAttrs(ctx={}){if(!studioEmbed||!Number.isInteger(ctx.blockIndex))return '';const design=ctx.block?.design||{};const parts=[`data-design-page="${Number.isInteger(ctx.pageIndex)?ctx.pageIndex:state.pageIndex}"`,`data-design-block="${ctx.blockIndex}"`,`data-design-x="${clampNumber(design.x,-240,240,0)}"`,`data-design-y="${clampNumber(design.y,-240,240,0)}"`,`data-design-rotate="${clampNumber(design.rotate,-180,180,0)}"`,`data-design-scale="${clampNumber(design.scale,.5,1.8,1)}"`];if(ctx.blockId)parts.push(`data-block-id="${escapeHtml(ctx.blockId)}"`);if(Number.isInteger(ctx.columnIndex))parts.push(`data-design-column="${ctx.columnIndex}"`);if(Number.isInteger(ctx.childIndex))parts.push(`data-design-child="${ctx.childIndex}"`);return ` ${parts.join(' ')}`;}

...
export function publicationReadinessLabel(status){if(status?.canPublish&&status?.formalReady===false)return '严格门禁通过 · 设备待验收（提示）';if(status?.canPublish)return '严格门禁通过 · 可以正式发布';if(status?.audit?.blockers)return `严格门禁阻断 ${status.audit.blockers} 项`;return '尚未满足正式发布条件';}

...
export function publicationReadinessLabel(status){if(status?.canPublish&&status?.formalReady===false)return '硬性门禁通过 · 设备为提示';if(status?.canPublish)return '可以发布';if(status?.audit?.blockers)return `阻断 ${status.audit.blockers} 项`;return '尚未就绪';}

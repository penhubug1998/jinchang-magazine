...
export function publicationReadinessLabel(status){if(status?.forceRelease)return '可直接发布 · 检查项仅作提示';if(status?.canPublish&&status?.formalReady===false)return '可以发布 · 设备检查仅作提示';if(status?.canPublish)return '可以直接发布';if(status?.audit?.blockers)return `检查提示 ${status.audit.blockers} 项`;return '可以继续制作';}

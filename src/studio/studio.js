...
// 横屏手机宽度会超过 820（见 mobile-studio.js 的说明），矮视口同样按手机界面处理。
const MOBILE_STUDIO_HEIGHT_BREAKPOINT = MOBILE_STUDIO?.MOBILE_STUDIO_HEIGHT_BREAKPOINT || 500;
...
const PEER_STARTED_AT = Date.now();
const PEER_LOCK_STALE_MS = 15000;
const PEER_HEARTBEAT_MS = 4000;
const appUrl = (url='') => {
...
const MEDIA_UPLOAD_LIMITS = Object.freeze({image:8*1024*1024,video:200*1024*1024,music:20*1024*1024,tts:8*1024*1024});
const mediaUploadLabel = bytes => `${(Number(bytes||0)/1024/1024).toFixed(0)} MB`;
const mediaKindLabelForUpload = kind => ({image:'图片',video:'视频',music:'音乐',tts:'朗读音频'})[kind]||'媒体';
async function uploadAsset(file,kind) {
  const limit=MEDIA_UPLOAD_LIMITS[kind];
  if(limit&&Number(file?.size||0)>limit)throw new Error(`${mediaKindLabelForUpload(kind)}文件超过 ${mediaUploadLabel(limit)} 上传上限，请先压缩后再上传`);
  ...
  const text=await r.text(); let data={}; try{data=text?JSON.parse(text):{}}catch{data={error:''}};
  let message=data.error||`${r.status} ${r.statusText}`;
  if(r.status===507||data.code==='STORAGE_FULL')message='服务器存储空间不足，素材没有保存。请清理服务器空间后重试。';
  else if(r.status===413)message=data.error||`${mediaKindLabelForUpload(kind)}文件超过 ${mediaUploadLabel(limit||0)} 上传上限，请先压缩后再上传`;
  else if(r.status>=500&&!data.error)message='服务器暂时无法保存素材，请稍后重试或联系管理员检查存储空间。';
  if(!r.ok){const error=new Error(message);error.code=data.code||'';error.status=r.status;throw error;}
  return data;
}
...
function peerEditTarget(target){return target?.closest?.('#pageEditor .page-meta-card,#pageEditor .content-editor-toolbar,#pageEditor #visualEditor,#pageEditor #jsonEditor,#pageEditor .quick-format-bar');}
function bindPeerReadOnlyGuard(){
  const blocked=e=>{if(!state.peerReadOnly||e.target?.closest?.('#peerReadOnlyBanner'))return;const area=peerEditTarget(e.target);if(!area)return;const control=e.target.closest?.('button,input,select,textarea,[contenteditable="true"]');if(!control)return;e.preventDefault();e.stopImmediatePropagation();if(e.type==='click')toast('当前窗口为只读，请先接管编辑权',2400);};
  for(const type of ['beforeinput','change','click','keydown'])document.addEventListener(type,blocked,true);
  window.addEventListener('pagehide',()=>{if(state.peerChannel&&state.issue)broadcastPeer('lock-release');},{once:false});
}
...
async function saveDraftNow() {
  if(state.peerReadOnly)return false;
...
function updateStateBadges() {
  ...
  const undo=$('#undoBtn'),redo=$('#redoBtn'),discard=$('#discardBtn'); if(undo)undo.disabled=state.peerReadOnly||!state.undoStack.length; if(redo)redo.disabled=state.peerReadOnly||!state.redoStack.length; if(discard)discard.disabled=state.peerReadOnly||!state.dirty;
...
function markDirty({ preview = true, readerSync = true, historyGroup='edit', forceHistory=false, recordHistory=true, assumeDirty=false, ensureIdentity=true } = {}) { if(state.peerReadOnly){toast('当前窗口为只读，请先接管编辑权',2400);return false;} if(ensureIdentity)ensureIssueIdentity(state.issue); if(!String(historyGroup).startsWith('design'))clearDesignHistory(); if(recordHistory)captureHistory(historyGroup,{force:forceHistory}); state.dirty = assumeDirty?true:!issuesEqual(state.issue,state.originalIssue); state.builtPreviewStale = true; if (state.audit) state.auditStale = true; updateStateBadges(); scheduleFieldStatsRefresh(); updateBuiltPreviewState(); scheduleDraftSave(); if(readerSync)scheduleReaderPreviewSync(); if (preview) renderPreview(); return true; }
...
function undoHistory(){ if(state.peerReadOnly)return toast('当前窗口为只读，请先接管编辑权',2400); if(!state.undoStack.length)return; const current=cloneData(state.issue),prev=state.undoStack.pop(); state.redoStack.push(current); applyHistorySnapshot(prev,{fromUndo:true}); updateStateBadges(); }
function redoHistory(){ if(state.peerReadOnly)return toast('当前窗口为只读，请先接管编辑权',2400); if(!state.redoStack.length)return; const current=cloneData(state.issue),next=state.redoStack.pop(); state.undoStack.push(current); applyHistorySnapshot(next,{fromUndo:false}); updateStateBadges(); }
...
async function openIssue(id,{skipDraftRecovery=false}={}) {
  ...
  const run=(async()=>{setIssueLoading(issueId);try{
    ...
    deferStudioTask(()=>loadSnapshots(state.issue.id)); [...document.querySelectorAll('.issue-item')].forEach((b,i) => b.classList.toggle('active', state.issues[i]?.id === issueId)); updateStateBadges();
    // 手机上不需要额外点一次「进入工作区」：选完一期直接落到手机编辑界面。
    // 桌面端保持原样（管理页 + 进入工作区按钮）；工作区内不会再跳转。
    if(MOBILE_STUDIO?.shouldAutoEnterWorkspace?MOBILE_STUDIO.shouldAutoEnterWorkspace({workspaceMode:WORKSPACE_MODE,width:window.innerWidth,height:window.innerHeight,entering:mobileAutoEntering}):(!WORKSPACE_MODE&&phoneSizedViewport()&&!mobileAutoEntering)){mobileAutoEntering=true;deferStudioTask(()=>{void enterWorkspaceFromManager();});}
    return true;
...
function peerRecordStale(record,now=Date.now()){return !record||now-Number(record.lastAt||0)>PEER_LOCK_STALE_MS;}
function peerRank(record){return [Number(record?.startedAt||Number.MAX_SAFE_INTEGER),String(record?.session||'')];}
function peerRankBefore(a,b){const [at,as]=peerRank(a),[bt,bs]=peerRank(b);return at<bt||(at===bt&&as<bs);}
function renderPeerLockState(){const locked=Boolean(state.peerReadOnly),banner=$('#peerReadOnlyBanner'),text=$('#peerReadOnlyText'),button=$('#peerTakeoverBtn'),owner=state.peers.get(state.peerLockOwner),stale=peerRecordStale(owner);document.body.classList.toggle('peer-readonly',locked);banner?.classList.toggle('hidden',!locked);if(text)text.textContent=stale?'原编辑窗口已失联，可以申请接管编辑权。':'为避免旧窗口覆盖新版本，本窗口暂时只读；可以浏览、检查和预览。';if(button){button.disabled=!stale;button.textContent=stale?'申请接管编辑':'等待其他窗口退出';}for(const id of ['saveBtn','workspaceSaveBtn']){const el=$('#'+id);if(el)el.disabled=locked||state.saving;}for(const id of ['undoBtn','redoBtn','discardBtn']){const el=$('#'+id);if(el&&locked)el.disabled=true;}}
function updatePeerIndicator(){const el=$('#workspacePeerState');if(!el)return;const count=state.peers?.size||0;el.classList.toggle('has-peer',count>0&&!state.remoteSavedIssue&&!state.peerReadOnly);el.classList.toggle('remote-pending',Boolean(state.remoteSavedIssue));el.classList.toggle('readonly',Boolean(state.peerReadOnly));el.classList.toggle('owner',Boolean(!state.peerReadOnly&&count>0));el.textContent=state.peerReadOnly?'只读 · 其他窗口编辑':state.remoteSavedIssue?'其他窗口已保存':count?`编辑中 · ${count+1} 窗口`:'单窗口';renderPeerLockState();}
function reconcilePeerLock(){if(!state.peerChannel||!state.issue){state.peerReadOnly=false;state.peerLockOwner='';renderPeerLockState();return;}const now=Date.now(),self={session:PEER_SESSION_ID,startedAt:state.peerClaimStartedAt||PEER_STARTED_AT,lastAt:now};for(const [session,peer] of state.peers)if(peerRecordStale(peer,now))state.peers.delete(session);const candidates=[self,...state.peers.values()];const winner=candidates.reduce((best,current)=>peerRankBefore(current,best)?current:best,self);const wasReadOnly=state.peerReadOnly;state.peerLockOwner=winner.session;state.peerLockStartedAt=winner.startedAt;state.peerLockLastAt=winner.lastAt;state.peerReadOnly=winner.session!==PEER_SESSION_ID;updatePeerIndicator();if(!wasReadOnly&&state.peerReadOnly)toast('其他窗口正在编辑，本窗口已切换为只读',3200);}
function closePeerChannel(){try{if(state.peerChannel&&state.issue)broadcastPeer('lock-release');state.peerChannel?.close?.()}catch{}clearInterval(state.peerHeartbeatTimer);clearInterval(state.peerLockSweepTimer);state.peerHeartbeatTimer=0;state.peerLockSweepTimer=0;state.peerChannel=null;state.peerIssueId='';state.peers=new Map();state.remoteSavedIssue=null;state.peerReadOnly=false;state.peerLockOwner='';updatePeerIndicator();}
function broadcastPeer(type,payload={}){const ch=state.peerChannel;if(!ch||!state.issue)return;try{ch.postMessage({source:'v3-studio-peer',session:PEER_SESSION_ID,issueId:state.issue.id,type,page:state.page,dirty:state.dirty,at:Date.now(),sessionStartedAt:state.peerClaimStartedAt||PEER_STARTED_AT,lockStartedAt:state.peerClaimStartedAt||PEER_STARTED_AT,...payload});}catch{}}
...
function handlePeerMessage(data){if(!data||data.source!=='v3-studio-peer'||data.session===PEER_SESSION_ID||data.issueId!==state.issue?.id)return;if(data.type==='lock-release'){state.peers.delete(data.session);reconcilePeerLock();return;}state.peers.set(data.session,{session:data.session,page:Number(data.page)||0,lastAt:Number(data.at)||Date.now(),dirty:Boolean(data.dirty),startedAt:Number(data.sessionStartedAt||data.lockStartedAt)||Number(data.at)||Date.now()});reconcilePeerLock();if(data.type==='hello')broadcastPeer('hello-ack');if(data.type==='saved'&&data.issue){if(state.dirty){state.remoteSavedIssue=cloneData(data.issue);state.remoteSavedAt=Number(data.at)||Date.now();updatePeerIndicator();toast('其他窗口已保存新版本；当前窗口有未保存修改，未自动覆盖',3600);}else applyRemoteSavedIssue(data.issue,{quiet:true});}}
function requestPeerTakeover(){if(!state.peerReadOnly)return;const owner=state.peers.get(state.peerLockOwner);if(!peerRecordStale(owner))return toast('其他窗口仍在编辑，请先关闭该窗口；如已关闭，请稍后再试',3200);state.peerClaimStartedAt=Date.now();state.peerReadOnly=false;state.peerLockOwner=PEER_SESSION_ID;broadcastPeer('lock-claim');reconcilePeerLock();toast('已接管编辑权');}
function initPeerChannel(issueId){closePeerChannel();if(!issueId||typeof BroadcastChannel!=='function')return;state.peerClaimStartedAt=PEER_STARTED_AT;try{const ch=new BroadcastChannel(`jinchang-magazine-v31-${issueId}`);state.peerChannel=ch;state.peerIssueId=issueId;ch.onmessage=e=>handlePeerMessage(e.data);broadcastPeer('hello');broadcastPeer('lock-claim');state.peerHeartbeatTimer=setInterval(()=>broadcastPeer('heartbeat'),PEER_HEARTBEAT_MS);state.peerLockSweepTimer=setInterval(reconcilePeerLock,PEER_HEARTBEAT_MS);reconcilePeerLock();}catch{closePeerChannel();}}
...
function visualHealthInfo(metric=currentVisualMetric()){const blocks=(currentPage()?.blocks||[]).length;if(!metric)return {state:'unknown',label:'版面 --',advice:'等待真实 Reader 完成当前页测量。',mode:'standard',fill:0,blocks,overflow:false,toolbarOccluded:false};const fill=Math.max(0,Math.min(1,Number(metric.fillRatio)||0)),overflow=Boolean(metric.overflow),toolbarOccluded=Boolean(metric.toolbarOccluded),obstructionAdvice=toolbarOccluded?' 预览底部可能被 Reader 工具栏遮挡，请收起工具栏、增加页内底部留白或拆分页面。':'';if(overflow||toolbarOccluded||fill>.96||blocks>12)return {state:'dense',label:toolbarOccluded?`版面 ${Math.round(fill*100)}% · 底部遮挡`:`版面 ${Math.round(fill*100)}% · 偏密`,advice:`当前页接近或超过可用高度。建议收紧段落/块间距；需要时可用工具栏“拆分到新页”将选中的内容块移动到续页。${obstructionAdvice}`,mode:'compact',fill,blocks,overflow,toolbarOccluded};if(fill<.38&&blocks<=3&&!['cover','toc','closing'].includes(currentPage()?.type))return {state:'warn',label:`版面 ${Math.round(fill*100)}% · 留白多`,advice:'当前页内容占用偏低。可适当放宽间距；如果只有少量短块，建议与相邻同栏目页面重新平衡。',mode:'relaxed',fill,blocks,overflow,toolbarOccluded};return {state:'good',label:`版面 ${Math.round(fill*100)}% · 正常`,advice:'当前页内容高度与密度处于正常范围。',mode:'standard',fill,blocks,overflow,toolbarOccluded};}
function updateVisualHealth(){const b=$('#visualHealthBtn'),x=visualHealthInfo();if(b){b.textContent=x.label;b.className=`visual-health-badge ${x.state==='good'?'good':x.state==='dense'?'dense':x.state==='warn'?'warn':''}`;}if($('#visualHealthDialog')?.open){const m=$('#visualHealthMetrics');m.innerHTML=`<div><span>内容占用</span><b>${Math.round(x.fill*100)}%</b></div><div><span>内容块</span><b>${x.blocks}</b></div><div><span>溢出</span><b>${x.overflow?'是':'否'}</b></div><div><span>底部遮挡</span><b>${x.toolbarOccluded?'是':'否'}</b></div><div><span>建议间距</span><b>${x.mode==='compact'?'紧凑':x.mode==='relaxed'?'宽松':'标准'}</b></div>`;$('#visualHealthAdvice').innerHTML=`<strong>${x.label}</strong>${escText(x.advice)}`;$('#visualHealthBalance').disabled=x.state==='unknown';const split=$('#visualHealthSplit');if(split)split.disabled=!selectedPageSplitPlan().ok;}if(WORKSPACE_MODE&&!validSelectedBlockIndices().length)renderContextInspector();else if(!WORKSPACE_MODE)updateManagerDashboard();}
function openVisualHealth(){updateVisualHealth();$('#visualHealthDialog')?.showModal();}
// 手机尺寸：窄（竖屏手机/平板竖屏）或矮（手机横屏）。
function phoneSizedViewport(){return MOBILE_STUDIO?.isPhoneSizedViewport?MOBILE_STUDIO.isPhoneSizedViewport(window.innerWidth,window.innerHeight):(window.innerWidth<=MOBILE_STUDIO_BREAKPOINT||window.innerHeight<=MOBILE_STUDIO_HEIGHT_BREAKPOINT);}
function mobileStudioViewportActive(){const force=new URLSearchParams(location.search).get('mobile')==='1';return Boolean(WORKSPACE_MODE&&(force||phoneSizedViewport()));}
// 手机上打开一期后自动进入工作区（只触发一次，避免重复跳转）。
let mobileAutoEntering=false;
...
function commitPendingMobileSheetText(){const ta=document.querySelector('#mobileSheetBody [data-mobile-quick-text]');if(!ta)return false;const card=ta.closest('[data-mobile-block-id]'),id=card?.dataset.mobileBlockId,field=ta.dataset.mobileField;if(!id||!field)return false;return mobileSaveText(id,field,ta.value);}function mobileSaveText(id,field,value){const i=findBlockIndexById(currentPage(),id),block=currentPage()?.blocks?.[i];if(i<0||!block||block.richText)return false;const safe=String(value??'').slice(0,12000);dispatchStudioCommand('block:update',{blockId:id,changes:{[field]:safe}},{historyGroup:'mobile-text',forceHistory:true,preview:false});renderPreview();pushReaderPreview().catch(()=>{});renderMobileStudio();toast('文字已更新');return true;}
...
$('#batchBlocksUp')?.addEventListener('click',()=>moveSelectedBlocks(-1));$('#batchBlocksDown')?.addEventListener('click',()=>moveSelectedBlocks(1));$('#batchBlocksSplit')?.addEventListener('click',openSplitPageDialog);$('#visualHealthSplit')?.addEventListener('click',openSplitPageDialog);$('#confirmSplitPage')?.addEventListener('click',moveSelectedBlocksToNewPage);$('#splitPageDialog')?.addEventListener('close',()=>{state.pageMovePreview=null;});$('#batchBlocksDuplicate')?.addEventListener('click',duplicateSelectedBlocks);$('#batchBlocksDelete')?.addEventListener('click',deleteSelectedBlocks);$('#quickAddLayout')?.addEventListener('click',()=>addBlock('container'));$('#quickAddBlock')?.addEventListener('click',openBlockDialog);$('#quickAdvancedDesign')?.addEventListener('click',quickAdvancedDesign);
...
function isMediaSourceMissing(value){const text=String(value??'').trim();return !text||/^(?:[（(]\s*)?(图片|视频|image|video|待补|占位|placeholder)(?:\s*[）)])?$/i.test(text);}
function mediaNeeds(){const out=[];(state.issue?.pages||[]).forEach((page,pageIndex)=>{const visit=(blocks=[],parent={})=>(blocks||[]).forEach((block,blockIndex)=>{if(!block||typeof block!=='object')return;const rootBlockIndex=Number.isInteger(parent.rootBlockIndex)?parent.rootBlockIndex:blockIndex;if(['image','video'].includes(block.type)&&isMediaSourceMissing(block.src))out.push({pageIndex,pageId:page.id,pageTitle:page.navTitle||page.title||`第 ${pageIndex+1} 页`,blockIndex:rootBlockIndex,columnIndex:parent.columnIndex,childIndex:parent.childIndex,nested:Boolean(parent.nested),kind:block.type,field:'src',label:block.caption||block.alt||`${mediaKindLabel(block.type)}占位`,blockId:block.id});if(block.type==='container'&&!parent.nested)(block.columns||[]).forEach((column,columnIndex)=>(column?.blocks||[]).forEach((child,childIndex)=>visit([child],{nested:true,rootBlockIndex,columnIndex,childIndex})));});visit(page.blocks||[]);});return out;}
function pendingMediaTargetFromElement(element){if(!element)return null;const pageIndex=Number(element.dataset.pageIndex),blockIndex=Number(element.dataset.blockIndex);if(!Number.isInteger(pageIndex)||!Number.isInteger(blockIndex))return null;const target={pageIndex,blockIndex,kind:element.dataset.mediaKind||'image',field:element.dataset.mediaField||'src'};if(element.dataset.nested==='1'){target.nested=true;target.columnIndex=Number(element.dataset.columnIndex);target.childIndex=Number(element.dataset.childIndex);}return target;}
function focusPendingMedia(target,{openReplace=false}={}){if(!target)return false;state.page=Math.max(0,Math.min(Number(target.pageIndex)||0,(state.issue?.pages?.length||1)-1));renderPages();renderPage();if(openReplace)return openMediaDialog(target);if(Number.isInteger(target.blockIndex))focusBlock(target.blockIndex);return true;}
function renderPendingMediaList(){const box=$('#mediaList'),needs=mediaNeeds();if(!box)return;if(!needs.length){box.innerHTML='<div class="media-empty">当前没有待补素材，图片和视频资源已全部绑定。</div>';renderMediaInspector();return;}box.innerHTML=needs.map(need=>`<article class="pending-media-item"><div class="pending-media-copy"><strong>第 ${need.pageIndex+1} 页 · ${escText(need.pageTitle)}</strong><span>${escText(mediaKindLabel(need.kind))} · ${escText(need.label)}</span><small>${need.nested?'容器内内容块':'顶层内容块'} · ${need.blockId?`ID ${escText(need.blockId)}`:'未设置稳定 ID'}</small></div><div class="pending-media-actions"><button type="button" data-pending-action="locate" data-page-index="${need.pageIndex}" data-block-index="${need.blockIndex}" data-column-index="${need.columnIndex??''}" data-child-index="${need.childIndex??''}" data-nested="${need.nested?'1':'0'}" data-media-kind="${need.kind}" data-media-field="${need.field}">定位页面</button><button type="button" class="primary" data-pending-action="replace" data-page-index="${need.pageIndex}" data-block-index="${need.blockIndex}" data-column-index="${need.columnIndex??''}" data-child-index="${need.childIndex??''}" data-nested="${need.nested?'1':'0'}" data-media-kind="${need.kind}" data-media-field="${need.field}">选择${escText(mediaKindLabel(need.kind))}</button></div></article>`).join('');renderMediaInspector();}
function blockSummary(block) {
  ...
  if (block.type === 'image' || block.type === 'video') return block.caption || block.src || (isMediaSourceMissing(block.src)?'待补素材':'');
...
// 属性面板的事件委托：面板内容每次都是 innerHTML 重建的，所以监听挂在容器上。
// 结构、批量属性、图层列表都走这一条通路。
function bindContextInspector(){
  const root=$('#contextInspector');if(!root||root.dataset.bound==='1')return;root.dataset.bound='1';
  root.addEventListener('click',e=>{
    const layerRow=e.target.closest('[data-layer-block]');
    if(layerRow){const index=Number(layerRow.dataset.layerBlock);if(Number.isInteger(index)){setBlockSelection([index],{anchor:index});syncCanvasSelectionToReader();}return;}
    const button=e.target.closest('[data-inspector-action]');
    if(button){e.preventDefault();handleInspectorAction(button.dataset.inspectorAction);return;}
  });
  root.addEventListener('change',e=>{
    const field=e.target.closest('[data-inspector-design]');
    if(!field)return;applyInspectorDesign(field.dataset.inspectorDesign,field.value);
  });
  root.addEventListener('input',e=>{
    const field=e.target.closest('input[type="range"][data-inspector-design]');
    if(!field)return;applyInspectorDesign(field.dataset.inspectorDesign,field.value);
  });
}
function handleInspectorAction(action){const row=selectedBlockObjects()[0],block=row?.block,index=row?.index;if(action==='health')return openVisualHealth();if(action==='balance')return balanceCurrentPageSpacing();if(action==='layout')return openLayoutLab();if(action==='page-design')return openDesignDialog('page');if(action==='duplicate')return duplicateSelectedBlocks();if(action==='delete')return deleteSelectedBlocks();if(action==='replace-media'&&block)return openMediaDialog({kind:block.type==='video'?'video':'image',blockIndex:index,field:'src'});if(action==='image-adjust'&&block?.type==='image')return openImageAdjust(index);if(action==='video-poster'&&block?.type==='video')return openMediaDialog({kind:'image',blockIndex:index,field:'poster'});if(action==='image-fit'&&block?.type==='image'){block.fit=block.fit==='cover'?'contain':'cover';mutateBlocks();return;}if(action==='image-ratio'&&block?.type==='image'){const seq=['auto','16:9','4:3','3:2','1:1'],at=Math.max(0,seq.indexOf(block.frameRatio||'auto'));block.frameRatio=seq[(at+1)%seq.length];mutateBlocks();return;}if(action==='align-left')return alignSelectedBlocks('left');if(action==='align-center')return alignSelectedBlocks('center');if(action==='align-right')return alignSelectedBlocks('right');if(action==='align-top')return alignSelectedBlocksVertical('top');if(action==='align-middle')return alignSelectedBlocksVertical('middle');if(action==='align-bottom')return alignSelectedBlocksVertical('bottom');if(action==='distribute')return distributeSelectedBlocks();if(action==='layer-up')return setSelectedCanvasLayer('forward');if(action==='layer-down')return setSelectedCanvasLayer('backward');if(action==='layer-reset'){const rows=canvasAlignTargets();if(!rows.length)return toast('请先选择内容块');let n=0;for(const row of rows){if(row.block?.design){const d={...row.block.design};delete d.z;canvasWriteDesign(row.block,d);n++;}}if(!n)return;markDirty({preview:false,historyGroup:'canvas-layer-reset',forceHistory:true});syncJsonFromPage();renderBlockList();renderPreview();pushReaderPreview().catch(()=>{});return toast(`已重置层级 · ${n} 个`);}}
...
function splitPageBaseTitle(page){return String(page?.title||page?.navTitle||'新页面').replace(/（续\d+）$/,'').trim()||'新页面';}
function nextContinuationNumber(source){const base=splitPageBaseTitle(source),pages=Array.isArray(state.issue?.pages)?state.issue.pages:[];let max=0;for(const page of pages){if(page===source)continue;const title=String(page?.title||page?.navTitle||'');if(!title.startsWith(`${base}（续`))continue;const match=title.match(/（续(\d+)）$/);if(match)max=Math.max(max,Number(match[1])||0);}return max+1;}
function buildContinuationPage(source){const page=newBlankPage(),number=nextContinuationNumber(source),base=splitPageBaseTitle(source),navBase=String(source?.navTitle||base).replace(/（续\d+）$/,'').trim()||base;page.type=['cover','toc','closing'].includes(source?.type)?'article':source?.type||'article';page.navTitle=`${navBase}（续${number}）`;page.section=source?.section||'';page.kicker=source?.kicker||'';page.title=`${base}（续${number}）`;if(source?.publishing)page.publishing=cloneData(source.publishing);if(source?.design)page.design=cloneData(source.design);return page;}
function splitBlockLabel(block,index){const kind=BLOCK_NAMES[block?.type]||block?.type||'内容块',summary=String(blockSummary(block)||'暂无内容').replace(/\s+/g,' ').trim();return `${String(index+1).padStart(2,'0')} · ${kind}${summary?` · ${summary}`:''}`.slice(0,180);}
function selectedPageSplitPlan(){const page=currentPage(),blocks=page?.blocks||[],indices=validSelectedBlockIndices();if(!page)return {ok:false,message:'请先选择可编辑期刊'};if(!blocks.length)return {ok:false,message:'当前页没有内容块'};if(!indices.length)return {ok:false,message:'请先选择要拆分的内容块'};if(['cover','toc','closing'].includes(page.type))return {ok:false,message:'封面、目录和尾页不参与内容拆分'};if(indices.length>=blocks.length)return {ok:false,message:'不能将当前页全部内容移到新页，请至少保留一个块'};if(!Array.isArray(state.issue?.pages)||state.issue.pages.length>=LIMITS.pages)return {ok:false,message:`页面已达到制作中心上限 ${LIMITS.pages} 页`};const selected=indices.map(index=>blocks[index]),selectedIds=selected.map(block=>block?.id);if(selectedIds.some(id=>!id))return {ok:false,message:'选中的内容块缺少稳定 ID，请先保存或刷新后重试'};const selectedSet=new Set(indices),remaining=blocks.filter((_,index)=>!selectedSet.has(index));return {ok:true,sourceIndex:state.page,sourceId:page.id,sourceTitle:page.navTitle||page.title||`第 ${state.page+1} 页`,target:buildContinuationPage(page),selectedIndices:indices,selectedIds,selected,remaining};}
function renderSplitPageDialog(plan){if(!plan?.ok)return;const sourceTitle=$('#splitPageSourceTitle'),targetTitle=$('#splitPageTargetTitle'),summary=$('#splitPageDialogSummary'),sourceList=$('#splitPageSourceList'),targetList=$('#splitPageTargetList'),confirmButton=$('#confirmSplitPage');if(sourceTitle)sourceTitle.textContent=`第 ${plan.sourceIndex+1} 页 · ${plan.sourceTitle}`;if(targetTitle)targetTitle.textContent=`新页 · ${plan.target.navTitle||plan.target.title||'续页'}`;if(summary)summary.textContent=`将 ${plan.selected.length} 个内容块移动到新续页；原文与内容块 ID 保持不变，不复制内容。`;if(sourceList)sourceList.innerHTML=plan.remaining.length?plan.remaining.map((block,index)=>`<div class="split-page-item muted"><b>保留</b><span>${escText(splitBlockLabel(block,index))}</span></div>`).join(''):'<div class="split-page-item muted"><b>提示</b><span>来源页将没有剩余内容块。</span></div>';if(targetList)targetList.innerHTML=plan.selected.map((block,index)=>`<div class="split-page-item"><b>移动</b><span>${escText(splitBlockLabel(block,plan.selectedIndices[index]))}</span></div>`).join('');if(confirmButton)confirmButton.disabled=false;}
function openSplitPageDialog(){const plan=selectedPageSplitPlan();if(!plan.ok)return toast(plan.message,3200);state.pageMovePreview=plan;renderSplitPageDialog(plan);$('#visualHealthDialog')?.close();const dialog=$('#splitPageDialog');if(dialog&&!dialog.open)dialog.showModal();return plan;}
function moveSelectedBlocksToNewPage(){const plan=state.pageMovePreview||selectedPageSplitPlan();if(!plan?.ok)return toast(plan?.message||'当前选择无法拆分',3200);const source=state.issue?.pages?.[plan.sourceIndex];if(!source||source.id!==plan.sourceId)return toast('当前页面已变化，请重新选择内容块',3200);const liveBlocks=source.blocks||[],selectedIdSet=new Set(plan.selectedIds),moved=liveBlocks.filter(block=>selectedIdSet.has(block?.id));if(moved.length!==plan.selectedIds.length||moved.length>=liveBlocks.length)return toast('内容块已变化，请重新打开拆分预览',3200);const target=cloneData(plan.target);target.blocks=moved;const insertAt=Math.min(state.issue.pages.length,Math.max(0,plan.sourceIndex+1)),closing=state.issue.pages.findIndex(page=>page?.type==='closing'),actualAt=closing>=0&&insertAt>closing?closing:insertAt;source.blocks=liveBlocks.filter(block=>!selectedIdSet.has(block?.id));state.issue.pages.splice(actualAt,0,target);state.page=actualAt;state.readerTargetPage=actualAt;state.selectedBlocks=new Set(moved.map((_,index)=>index));state.selectedBlockIds=new Set(moved.map(block=>block.id));state.lastSelectedBlock=moved.length-1;state.pageMovePreview=null;$('#splitPageDialog')?.close();ensureIssueIdentity(state.issue);syncJsonFromPage();markDirty({historyGroup:'page-split',forceHistory:true});renderPages();renderPage();syncWorkspaceRoute();broadcastPeer('presence');requestAnimationFrame(()=>document.querySelector(`.page-item[data-page-index="${state.page}"]`)?.scrollIntoView({block:'nearest'}));toast(`已将 ${moved.length} 个内容块移动到第 ${actualAt+1} 页`);return true;}
// —— PPT 式画布编辑：复制 / 粘贴 / 原位复制 / 层级 ——
// 画布模式下面向"自由编辑"的最小命令集。所有改动都走同一条历史链，
// 一次操作一个撤销点。
function selectedBlockRows(){return validSelectedBlockIndices();}
function copySelectedBlocks(){
  const blocks=currentPage()?.blocks||[],rows=selectedBlockRows();
  if(!rows.length)return toast('请先选择内容块');
  state.canvasClipboard=rows.map(i=>cloneData(blocks[i])).filter(Boolean);
  toast(`已复制 ${state.canvasClipboard.length} 个内容块`);
  return state.canvasClipboard.length;
}
function pasteCanvasClipboard(offset={x:18,y:18}){
  const blocks=currentPage()?.blocks||[],clip=state.canvasClipboard||[];
  if(!clip.length){toast('剪贴板为空，先按 ⌘/Ctrl+C 复制');return false;}
  if(blocks.length+clip.length>LIMITS.blocksPerPage){toast(`粘贴后超过单页 ${LIMITS.blocksPerPage} 个内容块`);return false;}
  const inserted=[],ids=[];
  for(const source of clip){
    const copy=cloneData(source);
    regenerateBlockIdentity(copy);
    const design={...(copy.design||{})};
    design.x=Math.round((Number(design.x)||0)+offset.x);
    design.y=Math.round((Number(design.y)||0)+offset.y);
    copy.design=design;
  ...
  state.selectedBlocks=new Set(inserted);
  state.selectedBlockIds=new Set(ids);
  state.lastSelectedBlock=inserted[0]??null;
  mutateBlocks();
  if(state.canvasMode)pushReaderPreview().catch(()=>{});
  toast(`已粘贴 ${inserted.length} 个内容块`);
  return true;
}
function setSelectedCanvasLayer(direction){
  if(!state.canvasMode)return false;
  const blocks=currentPage()?.blocks||[],rows=selectedBlockRows();
  if(!rows.length)return toast('请先选择内容块');
  const current=blocks.reduce((max,b)=>Math.max(max,Number(b?.design?.z)||0),0);
  for(const index of rows){
    const block=blocks[index];if(!block)continue;
    const z=Number(block.design?.z)||0;
    const next=direction==='front'?current+1:direction==='back'?Math.min(0,z)-1:z+1;
    block.design={...(block.design||{}),z:Math.round(next)};
  }
  markDirty({preview:false,historyGroup:`canvas-layer-${direction}`,forceHistory:true});
  syncJsonFromPage();renderBlockList();renderPreview();pushReaderPreview().catch(()=>{});
  toast(direction==='front'?'已置于最上层':direction==='back'?'已置于最下层':'已上移一层');
  return true;
}
// —— 画布对齐 / 分布 / 层级 ——
// 对齐以页面内容区（`.page-body`）为参照；容器块与子块不参与，避免嵌套坐标系混用。
// 图层列表：把当前页内容按层级从高到低列出，点击即选中，可上下调层。
// 层级用 design.z 表达；全部为 0 时按文档顺序显示，避免"看不见层级"。
function layerPanelRows(){
  const page=currentPage();if(!page)return [];
  return (page.blocks||[]).map((block,index)=>({block,index,z:Number(block?.design?.z)||0}))
    .sort((a,b)=>b.z-a.z||a.index-b.index);
}
function renderLayerPanel(){
  const rows=layerPanelRows();if(!rows.length)return '';
  const items=rows.map(({block,index,z})=>{
    const selected=state.selectedBlocks.has(index);
    const label=BLOCK_NAMES[block?.type]||block?.type||'内容块';
    const text=String(block?.title||block?.text||block?.caption||'').replace(/\s+/g,' ').trim().slice(0,26)||'（无文字）';
    return `<li class="layer-row ${selected?'is-selected':''}" data-layer-block="${index}"><span class="layer-order">${String(index+1).padStart(2,'0')}</span><span class="layer-copy"><b>${escText(label)}</b><small>${escText(text)}</small></span><span class="layer-z">${z?`z ${z}`:'—'}</span></li>`;
  }).join('');
  return `<section class="context-section"><span class="context-kicker">LAYERS</span><ul class="layer-list">${items}</ul><div class="context-action-grid"><button type="button" data-inspector-action="layer-up">上移一层</button><button type="button" data-inspector-action="layer-down">下移一层</button></div><div class="context-action-grid"><button type="button" data-inspector-action="layer-reset">重置层级</button></div></section>`;
}
function canvasAlignTargets(){
  const page=currentPage();if(!page)return [];
  return selectedBlockObjects().filter(row=>row.block&&row.block.type!=='container');
}
function canvasDesignOf(block){return {...(block?.design||{})};}
function canvasWriteDesign(block,design){
  const next={...design};
  for(const key of Object.keys(next))if(next[key]===0||next[key]===null||next[key]===undefined||(key==='scale'&&next[key]===1)||(key==='rotate'&&next[key]===0))delete next[key];
  if(Object.keys(next).length)block.design=next;else delete block.design;
}
// 对齐用 design.x / design.y 统一表达：以选中块的极值/均值为目标。
// 位置归零会被 canvasWriteDesign 删除，所以对齐回原位不会留下偏移垃圾。
function alignSelectedBlocks(mode){
  const rows=canvasAlignTargets();if(rows.length<2)return toast('至少选择 2 个内容块再对齐');
  const xs=rows.map(r=>Number(r.block?.design?.x)||0);
  const target=mode==='left'?Math.min(...xs):mode==='center'?Math.round(xs.reduce((a,b)=>a+b,0)/xs.length):Math.max(...xs);
  let changed=0;
  for(const row of rows){
    const design=canvasDesignOf(row.block);
    const before=JSON.stringify(row.block.design||null);
    design.x=target;canvasWriteDesign(row.block,design);
    if(JSON.stringify(row.block.design||null)!==before)changed++;
  }
  if(!changed)return 0;
  markDirty({preview:false,historyGroup:`canvas-align-${mode}`,forceHistory:true});
  syncJsonFromPage();renderBlockList();renderPreview();pushReaderPreview().catch(()=>{});
  toast(`已按${mode==='left'?'左':mode==='center'?'水平居中':'右'}对齐 · ${changed} 个`);
  return changed;
}
// 垂直方向没有 alignSelf 语义，用相对第一个块的 y 差值对齐。
function alignSelectedBlocksVertical(mode){
  const rows=canvasAlignTargets();if(rows.length<2)return toast('至少选择 2 个内容块再对齐');
  const ys=rows.map(r=>Number(r.block?.design?.y)||0);
  const target=mode==='top'?Math.min(...ys):mode==='middle'?Math.round(ys.reduce((a,b)=>a+b,0)/ys.length):Math.max(...ys);
  let changed=0;
  for(const row of rows){
    const design=canvasDesignOf(row.block);
    const before=JSON.stringify(row.block.design||null);
    design.y=target;canvasWriteDesign(row.block,design);
    if(JSON.stringify(row.block.design||null)!==before)changed++;
  }
  if(!changed)return 0;
  markDirty({preview:false,historyGroup:`canvas-align-${mode}`,forceHistory:true});
  syncJsonFromPage();renderBlockList();renderPreview();pushReaderPreview().catch(()=>{});
  toast(`已按${mode==='top'?'顶':mode==='middle'?'垂直居中':'底'}对齐 · ${changed} 个`);
  return changed;
}
// 分布：把中间块的 y 均匀排布在首尾之间。
function distributeSelectedBlocks(){
  const rows=canvasAlignTargets();if(rows.length<3)return toast('至少选择 3 个内容块再分布');
  const sorted=rows.map((row,i)=>({row,y:Number(row.block?.design?.y)||0,i})).sort((a,b)=>a.y-b.y||a.i-b.i);
  const count=sorted.length,first=sorted[0].y,last=sorted[count-1].y;
  let changed=0;
  sorted.forEach((item,order)=>{
    if(order===0||order===count-1)return;
    const design=canvasDesignOf(item.row.block);
    const target=Math.round(first+((last-first)*order)/(count-1));
    const before=JSON.stringify(item.row.block.design||null);
    design.y=target;canvasWriteDesign(item.row.block,design);
    if(JSON.stringify(item.row.block.design||null)!==before)changed++;
  });
  if(!changed)return 0;
  markDirty({preview:false,historyGroup:'canvas-distribute',forceHistory:true});
  syncJsonFromPage();renderBlockList();renderPreview();pushReaderPreview().catch(()=>{});
  toast(`已垂直等距分布 · ${changed} 个`);
  return changed;
}
function canvasClipboardActive(){return Boolean(state.canvasMode&&(state.canvasClipboard||[]).length);}
...
function renderMediaList() {
  if(state.mediaFilter==='pending'){renderPendingMediaList();return;}
...
async function openMediaDialog(target=null) { if(!state.issue)return toast('请先选择期刊'); state.mediaTarget=target; state.mediaIntent=target?'replace':'asset';state.mediaFilter=target?.kind||'all';state.mediaSelected=null;const current=target?mediaTargetBlock(target)?.[target.field||'src']:null;if(current&&!isMediaSourceMissing(current))state.mediaSelected=current; [...$('#mediaFilters').querySelectorAll('button')].forEach(b=>b.classList.toggle('active',b.dataset.kind===state.mediaFilter)); updateMediaAccept();const dialog=$('#mediaDialog');if(dialog&&!dialog.open)dialog.showModal(); await loadMediaAssets(); }
...
$('#mediaList').addEventListener('click',e=>{const pending=e.target.closest('[data-pending-action]');if(pending){const target=pendingMediaTargetFromElement(pending);if(target)focusPendingMedia(target,{openReplace:pending.dataset.pendingAction==='replace'});return;}const item=e.target.closest('.media-item');if(!item)return;state.mediaSelected=item.dataset.path;renderMediaList();});
...
function updateBuiltPreviewState(){const el=$('#builtPreviewState');if(!el)return;if(!state.issue){el.textContent='尚未选择期刊';return;}const pageSync=readerPageSyncStatus(),pageNumber=pageSync.target+1,retry=$('#refreshBuiltPreview'),visual=visualHealthInfo(),occluded=Boolean(visual.toolbarOccluded);let detail='真实 Reader 已同步当前编辑稿',short='Reader 已同步',busy=false;if(pageSync.timeout){detail=`Reader 第 ${pageNumber} 页同步超时，可点击“修复预览”重试`;short='Reader 同步超时';busy=true;}else if(!state.readerPreviewReady){detail='Reader 正在加载…';short='Reader 加载中';busy=true;}else if(state.builtPreviewStale){detail='正在同步当前编辑稿…';short='Reader 内容同步中';busy=true;}else if(!pageSync.matched){detail=`Reader 正在切换到第 ${pageNumber} 页…`;short=`Reader 切换第 ${pageNumber} 页`;busy=true;}if(occluded&&!busy){detail+='；预览底部可能被 Reader 工具栏遮挡';short='Reader 已同步 · 底部遮挡';}if(retry)retry.hidden=!pageSync.timeout;el.textContent=detail;el.classList.toggle('stale',busy);if(state.previewMode==='built'){$('#previewOverflow').textContent=busy?'同步中':occluded?'底部可能被工具栏遮挡':'实时同步';$('#previewOverflow').className=`preview-status ${busy||occluded?'warning':'ok'}`;if($('#workspaceReaderState')){$('#workspaceReaderState').textContent=short;$('#workspaceReaderState').classList.toggle('busy',busy);}}}
...
// Reader 画布工具条转发的编辑命令（复制/粘贴/层级）。
window.addEventListener('message',e=>{const frame=$('#builtPreviewFrame'),data=e.data||{};if(!frame||e.source!==frame.contentWindow||data.source!=='v3-reader'||String(data.issueId)!==String(state.issue?.id)||data.type!=='canvas-command')return;
  if(!state.canvasMode)return;
  const blockId=data.blockId?String(data.blockId):'';
  if(blockId){const index=findBlockIndexById(currentPage(),blockId);if(index>=0){state.selectedBlocks=new Set([index]);state.selectedBlockIds=new Set([blockId]);state.lastSelectedBlock=index;}}
  const action=String(data.action||'');
  const handlers={copy:()=>copySelectedBlocks(),paste:()=>pasteCanvasClipboard(),duplicate:()=>duplicateSelectedBlocks(),'layer-front':()=>setSelectedCanvasLayer('front'),'layer-back':()=>setSelectedCanvasLayer('back'),'layer-forward':()=>setSelectedCanvasLayer('forward'),'layer-backward':()=>setSelectedCanvasLayer('backward')};
...
async function saveIssue({ silent=false }={}) {
  if(state.peerReadOnly){toast('当前窗口为只读，请先接管编辑权',2400);return false;}
...
function renderWorkspaceAudit(){const panel=$('#workspaceAuditPanel');if(!panel)return;const hard=state.audit?.blockers||[],tips=[...(state.audit?.warnings||[]),...(state.audit?.notes||[])],st=$('#workspaceAuditState'),summary=$('#workspaceAuditSummary');if(!state.audit){st.textContent='尚未检查';st.className='workspace-audit-state';summary.textContent=state.publicationStatus?.forceRelease?'当前为直接发布模式：硬性阻断只记录为提示，不阻断发布上线。':'硬性门禁会阻断发布；提示项只提供修复建议，不阻断继续制作。';$('#workspaceAuditHardCount').textContent='0';$('#workspaceAuditTipCount').textContent='0';$('#workspaceAuditHard').innerHTML='<div class="workspace-audit-empty">尚未运行检查。</div>';$('#workspaceAuditTips').innerHTML='<div class="workspace-audit-empty">尚未运行检查。</div>';return;}const stale=state.auditStale||state.dirty;st.textContent=stale?'审计已过期':state.audit.readiness==='ready'?'硬性检查通过':state.audit.readiness==='blocked'?(state.publicationStatus?.forceRelease?'存在硬性阻断 · 直接发布模式':'存在硬性阻断'):'有提示项';st.className=`workspace-audit-state ${stale?'warning':state.audit.readiness||'warning'}`;summary.textContent=stale?'内容已修改，请重新检查；旧结果不参与发布判断。':`硬性 ${hard.length} 项 · 提示 ${tips.length} 项 · ${state.audit.strictMode?'严格检查':'日常检查'}；${state.publicationStatus?.forceRelease?'当前为直接发布模式，硬性阻断也不阻断上线。':'提示项不阻断继续制作。'}`;$('#workspaceAuditHardCount').textContent=String(hard.length);$('#workspaceAuditTipCount').textContent=String(tips.length);const render=(rows,empty,label)=>rows.length?rows.slice(0,4).map((f,i)=>`<div class="workspace-audit-row"><strong title="${escText(f.message||f.title||'')}">${escText(f.title||f.code||label)}</strong><small>${escText(f.code||label)}</small>${f.location?`<button type="button" data-workspace-audit-locate="${escText(String(f.__index??i))}" data-workspace-audit-severity="${f.severity||''}">定位</button>`:''}</div>`).join('')+(rows.length>4?`<div class="workspace-audit-empty">还有 ${rows.length-4} 项，请打开“检查”查看全部。</div>`:''):empty;const hardIndexed=hard.map((f,i)=>({...f,__index:i})),tipIndexed=tips.map((f,i)=>({...f,__index:i}));$('#workspaceAuditHard').innerHTML=render(hardIndexed,'<div class="workspace-audit-empty">✓ 没有硬性阻断项。</div>','硬性检查');$('#workspaceAuditTips').innerHTML=render(tipIndexed,'<div class="workspace-audit-empty">✓ 没有提示项。</div>','提示项');}
...
function renderAudit(audit,meta={}) { state.audit = audit; state.auditStale = false; const card = $('#auditCard'); card.classList.add('hidden'); const hero = $('#auditHero'); hero.className = `audit-hero ${audit.readiness || 'warning'}`; // 直接发布模式（V3_FORCE_RELEASE）下审计阻断只是提示，不阻断上线。这里必须跟着实话实说，
// 否则同一屏会出现「存在发布阻断项」和「可直接发布 · 检查项仅作提示」两种相反结论。
  const forceRelease = Boolean(state.publicationStatus?.forceRelease);
  const label = audit.readiness === 'ready' ? '可以进入发布流程' : audit.readiness === 'blocked' ? (forceRelease ? '存在硬性阻断 · 当前为直接发布模式，不阻断上线' : '存在发布阻断项') : '可以继续制作，但有待处理项'; $('#auditState').textContent = label; $('#auditMessage').textContent = audit.readiness === 'ready' ? '当前审计未发现硬性阻断项。提示项不会阻断继续制作。正式发布仍应执行完整 release:check。' : `硬性阻断 ${audit.blockers?.length || 0} 项，提示 ${Number(audit.warnings?.length||0)+Number(audit.notes?.length||0)} 项。${forceRelease ? '服务当前配置为直接发布模式，这些阻断只记录为提示，不会阻止发布上线。' : ''}点击“定位”可直接回到具体字段或内容块。`; $('#auditScore').textContent = String(audit.score ?? '--'); const metrics = [['页面',audit.pageCount??'-'],['硬性阻断',audit.blockers?.length||0],['提示项',(audit.warnings?.length||0)+(audit.notes?.length||0)],['TTS',audit.narration?`${audit.narration.found}/${audit.narration.expected}`:'-'],['媒体',audit.media?`${audit.media.found}/${audit.media.referenced}`:'-']]; $('#auditMetrics').innerHTML = metrics.map(([k,v])=>`<div class="audit-metric"><span>${escText(k)}</span><b>${escText(v)}</b></div>`).join(''); $('#auditGenerated').textContent = meta.generatedAt ? `生成：${fmtTime(meta.generatedAt)}${meta.strict?' · 严格模式':''}` : ''; $('#auditReportLink').href = meta.htmlUrl ? appUrl(meta.htmlUrl) : '#'; $('#auditReportLink').classList.toggle('hidden',!meta.htmlUrl); renderAuditFindings(); renderWorkspaceAudit(); updateStateBadges(); updateAuditEntry(); }
...
$('#workspaceAuditRun')?.addEventListener('click',()=>runAudit(false));$('#workspaceAuditStrict')?.addEventListener('click',()=>runAudit(true));$('#workspaceAuditPanel')?.addEventListener('click',e=>{const b=e.target.closest('[data-workspace-audit-locate]');if(b){const f=workspaceAuditFindingFromButton(b);if(f){closeWorkspaceAuditDialog();locateFinding(f);}}});
...
function renderPublicationCenter(){
  ...
  $('#publicationMetrics').innerHTML=[
    ...
    publicationMetricCard('服务器空间',st.storage?.label||'未检查',st.storage?.advice||'发布前检查服务器剩余空间')
  ].join('');
  ...
  const storageHard=st.storage?.status==='critical'?[{title:'服务器存储空间不足',message:st.storage.advice,severity:'blocker',fix:'清理旧发布包或扩容后重新运行发布前检查。'}]:[];
  const storageAdvisory=['warning','unknown'].includes(st.storage?.status)?[{title:'服务器存储空间提示',message:st.storage.advice,severity:'advisory',fix:'建议先清理旧发布包，再上传媒体或生成发布包。'}]:[];
  const hard=st.forceRelease?[]:[...(st.audit?.findings||[]).filter(x=>x.severity==='blocker'||x.severity==='error'),...(Number(m.links?.invalid||0)>0?[{title:'不安全或格式错误链接',message:`${m.links.invalid} 个链接必须先修复`,severity:'blocker'}]:[]),...storageHard];
  const advisory=[...(st.audit?.findings||[]).filter(x=>!hard.includes(x)),...storageAdvisory];
...
function publicationFindingRow(f,kind,findings){
  ...
  const splitHint=Number.isInteger(Number(f.suggestedSplitAfter))?`<small>可直接在制作中心选择内容块后拆页：第 ${Number(f.suggestedSplitAfter)} 个内容块之后</small>`:'';
...
// PPT 式精确微调：画布模式下选中内容块后，方向键移动 1px，Shift 键 10px。
// 手感要求来自"像 PPT 那样自由编辑"，键盘对齐是最基础的一条。
function nudgeSelectedCanvasBlocks(dx,dy){
  if(!state.canvasMode||!state.issue)return false;
  const selected=[...state.selectedBlocks].filter(i=>Number.isInteger(i)).sort((a,b)=>a-b);
  if(!selected.length)return false;
  const page=currentPage();if(!page)return false;
  let changed=0;
  for(const index of selected){
    const block=page.blocks?.[index];if(!block)continue;
    const design={...(block.design||{})};
    design.x=Math.round((Number(design.x)||0)+dx);
    design.y=Math.round((Number(design.y)||0)+dy);
    if(!design.x)delete design.x;
    if(!design.y)delete design.y;
    if(Object.keys(design).length)block.design=design;else delete block.design;
    changed++;
  }
  if(!changed)return false;
  // 不传 forceHistory：同组且间隔 < 650ms 的操作合并为一个撤销点，
  // 这样连按方向键只产生一次可撤销的移动，而不是每 1px 一次。
  markDirty({preview:false,historyGroup:'canvas-nudge'});
  syncJsonFromPage();
  renderBlockList();
  renderPreview();
  pushReaderPreview().catch(()=>{});
  return true;
}
// 画布快捷键：⌘/Ctrl+C 复制、⌘/Ctrl+V 粘贴、⌘/Ctrl+D 原位复制、
// ⌘/Ctrl+[ / ] 下移 / 上移一层、⌘/Ctrl+Shift+[ / ] 置底 / 置顶。
document.addEventListener('keydown',e=>{
  ...
  if(key==='c'){if(e.shiftKey)return;if(copySelectedBlocks())e.preventDefault();return;}
  if(key==='v'){if(pasteCanvasClipboard())e.preventDefault();return;}
  if(key==='d'){if(duplicateSelectedBlocks())e.preventDefault();return;}
  if(key==='['||key===']'){
    const direction=key===']'?(e.shiftKey?'front':'forward'):(e.shiftKey?'back':'backward');
    if(setSelectedCanvasLayer(direction))e.preventDefault();
    return;
  }
});
document.addEventListener('keydown',e=>{
  ...
  if(nudgeSelectedCanvasBlocks(dx,dy)){e.preventDefault();}
});
